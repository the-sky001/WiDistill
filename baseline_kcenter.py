...
def kcenter_selection(data, labels, num_samples_per_class):
    selected_indices = []
    ...
    for cls in unique_classes:
        ...
        # 选择第一个中心点
        initial_center_index = np.random.choice(len(class_indices))
        centers = [class_indices[initial_center_index]]
        
        while len(centers) < num_samples_per_class:
            max_distance = -1
            new_center = None
            ...
            for idx in class_indices:
                if idx not in centers:
                    # 计算到所有现有中心点的最小距离
                    min_distance = np.min([np.linalg.norm(data[idx].reshape(-1) - data[center].reshape(-1)) for center in centers])
        ...
        selected_indices.extend(centers)
...
def main(args):
    ...
    for ch in range(channel):
        mean_val = torch.mean(images_all[:, ch]).item()
        std_val = torch.std(images_all[:, ch]).item()
        logging.info(f'real images channel {ch}, mean = {mean_val:.4f}, std = {std_val:.4f}')

    criterion = nn.CrossEntropyLoss().to(args.device)

    # 使用 k-center 方法选择每类的代表性样本
    num_samples_per_class = 100  # 设置每一类希望选择的样本数
    print(f"num_samples_per_class: {num_samples_per_class}")

    logging.info("Applying k-center selection...")
    start_time = time.time()
    selected_indices = kcenter_selection(images_all.numpy(), labels_all.numpy(), num_samples_per_class)
    logging.info(f"k-center selection applied in {time.time() - start_time:.2f} seconds")

    selected_features = images_all[selected_indices]
    selected_labels = labels_all[selected_indices]

    # 保存选择出的数据子集
    subset_save_path = os.path.join(args.buffer_path, 'kcenter_subset', args.dataset)
    ...
    logging.info(f"k-center subset saved to {subset_save_path}")

    # 打印最终生成子集的形状
    logging.info(f"Final subset shape: features {selected_features.shape}, labels {selected_labels.shape}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='K-center Selection')
    ...
    parser.add_argument('--log_file', type=str, default='./kcenter.log', help='path to log file')
