...
class MMFi_Dataset(Dataset):
    def __init__(self, data_base, data_unit, modality, split, data_form, wifi_transform=None):
        ...
        for subject, actions in self.data_source.items():
            for action in actions:
                if self.data_unit == 'sequence':
                    ...
                    # frame_num = 297
                    data_path = os.path.join(self.data_base.data_root, self.get_scene(subject), subject, action, self.modality[0])
                    mat_files = [f for f in os.listdir(data_path) if f.endswith('.mat')]
                    frame_num = len(mat_files)  # 动态设置 frame_num
    ...
    def read_frame(self, frame):
        ...
        if mod in ['infra1', 'infra2', 'rgb']:
            ...
            mean = np.array([0.4643, 0.4700, 0.4552]).reshape(3, 1, 1)
            std = np.array([0.3851, 0.3902, 0.3786]).reshape(3, 1, 1)

            # 标准化数据
            # data = (data - mean) / std 
            data = data.reshape(342, 350)
            # mean = -0.1822
            # std = 0.8553
            # data = (data - mean) / std
            
            if self.wifi_transform:
                data = self.wifi_transform.augment(data)
        else:
            raise ValueError('Found unseen modality in this dataset.')
        return data
        ...
        if self.data_unit == 'sequence':
            ...
            for mod in item['modality']:
                ...
                if os.path.isfile(data_path):
                    ...
                    # sample['input_'+mod] = data_mod
                    sample['input_'+mod] = torch.from_numpy(data_mod) 
                else:
                    raise ValueError('{} is not a file!'.format(data_path))
        else:
            raise ValueError('Unsupport data unit!')
        sample['action'] = torch.tensor(label_encoder.transform([sample['action']])[0], dtype=torch.long)
        # print(sample['action'])
        return sample['input_'+mod], sample['action']
...
def make_dataset(dataset_root, config):
    ...
    wifi_augmentation = WiFiDataAugmentation(noise_level=0.01, shift_range=2, scale_range=0.1)
    ...
    #     transforms.ToTensor()
    # ])
    train_dataset = MMFi_Dataset(database, config['data_unit'], **config_dataset['train_dataset'],wifi_transform=wifi_augmentation)
    val_dataset = MMFi_Dataset(database, config['data_unit'], **config_dataset['val_dataset'],wifi_transform=None)
